...
const Navbar = () => {
    const [showLogin, setShowLogin] = useState(false); // login form ke liye
    const [showSign, setShowSign] = useState(false);  // signup form ke liye
...
>>>>>>> 228af4e1b70531c6c8fff6514f7f5642713734d2
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (loginRef.current && !loginRef.current.contains(event.target) && !event.target.closest("#login-button")) {
                setShowLogin(false);
                setShowSign(false);
            }
                                ...
                                <div className="absolute top-10 right-0 bg-white border rounded shadow-md w-36 z-50">
                                    <ul className="text-sm">
                                        ...
                                        <li
                                            ...
                                            onClick={() => {
                                                ['token', 'userEmail', 'userId', 'userName'].forEach(key => {
